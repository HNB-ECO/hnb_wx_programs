...
(function ($) {
    ...
    var fixHeaderCSS = function (that) {
    	if (that.options.fixHeader) {
    		 that.$tableHeader.css('height', that.options.fixHeader);
		} else {
			that.$tableHeader.css('height', '72px');
		}
    };
            ...
            if (header.find('.date-filter-control').length > 0) {
                $.each(that.columns, function (i, column) {
                    ...
                    if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'daterangepicker') {
                    	var $datepicker = $(sprintf("#daterangepicker-%s", column.field));
                    	if ($datepicker.length > 0) {
                    		$datepicker.remove();
                    	}
                    	var $datepickerInput = header.find('#date-filter-control-' + column.field);
                    	$datepickerInput.daterangepicker(
							...
							function (start, end) {
								...
								var $input = $('#date-filter-control-' + column.field);
								$input.val(date);
                    	...
                    	$datepickerInput.val("");
                    	$(".daterangepicker:not([id])").attr("id", sprintf("daterangepicker-%s", column.field));
                    }
                });
            }
        } else {
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            input: function (that, field, isVisible, placeholder) {
                return sprintf('<input type="text" class="form-control input-sm bootstrap-table-filter-control-%s" style="width: 100%; text-align: center; visibility: %s" placeholder="%s">', field, isVisible, placeholder);
            },
            select: function (that, field, isVisible) {
                return sprintf('<select class="form-control input-sm bootstrap-table-filter-control-%s" style="width: 100%; visibility: %s" dir="%s"></select>',
                    field, isVisible, getDirectionOfSelectOptions(that.options.alignmentSelectControlOptions));
            },
            datepicker: function (that, field, isVisible) {
                return sprintf('<input type="text" id="date-filter-control-%s" class="form-control input-sm date-filter-control bootstrap-table-filter-control-%s" style="width: 100%; text-align: center; visibility: %s">', field, field, isVisible);
            },
            daterangepicker: function (that, field, isVisible) {
            	return sprintf('<input type="text" id="date-filter-control-%s" class="form-control input-sm date-filter-control bootstrap-table-filter-control-%s" style="width: 100%; text-align: center; visibility: %s">', field, field, isVisible);
            }
        },
    ...
    BootstrapTable.prototype.triggerSearch = function (event) {
    	var that = this;
        var header = getCurrentHeader(that),
            searchControls = getCurrentSearchControls(that);
        header.find(searchControls).each(function () {
            ...
            var $field = el.closest('[data-field]').data('field');
        ...
        // we're manually appending some text to bootrap's searchText field
        // to guarantee that it will perform a search again when we call this.onSearch(event)
        that.searchText += "randomText";
        that.options.pageNumber = 1;
        that.EnableControls(false);
        that.onSearch(event);
        that.trigger('column-search');
    };
