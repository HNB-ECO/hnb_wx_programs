...
(function ($) {
        ...
        $.each(this.options.columns, function (i, columns) {
            ...
            $.each(columns, function (j, column) {
                ...
                if (typeof column.fieldIndex !== 'undefined') {
                    ...
                    that.header.columnType[column.fieldIndex] = column.columnType;
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        // showColumns, showToggle, showRefresh, showTriggerSearch
        ...
        if (this.options.showTriggerSearch) {
            html.push(sprintf('<button class="btn' +
                    sprintf(' btn-%s', this.options.buttonsClass) +
                    sprintf(' btn-%s', this.options.iconSize) +
                    '" type="button" name="triggerSearch" aria-label="triggerSearch" title="%s">',
                    this.options.formatTriggerSearch()),
                sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.triggerSearch),
                '</button>');
        }
        ...
        if (this.options.showTriggerSearch) {
            this.$toolbar.find('button[name="triggerSearch"]')
                .off('click').on('click', $.proxy(this.triggerSearch, this));
        }
    ...
    BootstrapTable.prototype.initServer = function (silent, query, url) {
        ...
        if (this.options.searchObject) {
            params.searchObject = this.options.searchObject;
        }
        ...
        data["columns"] = JSON.stringify(this.header.fields)
        data["columnType"] = JSON.stringify(this.header.columnType)
        ...
        request = $.extend({}, calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            error: function (res) {
                ...
                if (that.options.sidePagination === 'server') {
                    ...
                    data[that.options.totalMapField] = {};
                }
                that.load(data);
...
//            html.push(calculateObjectValue(column, column.footerFormatter, [data], '&nbsp;') || '&nbsp;');
            ...
            if (column.showTotal) {
            	html.push(getTotalByKey(that, column.field));
            } else {
            	html.push('&nbsp;');
            }
    ...
    BootstrapTable.prototype.load = function (data) {
        ...
        if (this.options.pagination && this.options.sidePagination === 'server') {
            ...
            this.options.totalMap = data[this.options.totalMapField];
    ...
    BootstrapTable.prototype.getFilterColumnsPartial = function () {
        var that = this;
        return this.filterColumnsPartial
    };
    ...
    var getTotalByKey = function (that, key) {
    	if (!that || !that.options 
    			|| !that.options.totalMap || !key) {
    		return "0";
    	}
    	var total = that.options.totalMap[key];
    	if (!total) {
    		return "0";
    	}
    	total = total + "";
    ...
    var allowedMethods = [
        'getOptions',
        'getSelections', 'getAllSelections', 'getData','getFilterColumnsPartial',
    ...
    $.fn.bootstrapTable = function (option) {
        ...
        if (typeof option === 'object') {
        	var defaultOption = {
        			...
        			showTriggerSearch: true,//开启手动搜索
        			showColumns: true,//内容列下拉框
        			filterControl: true, //开起列搜索
        			filterShowClear: true, //显示清空搜索按钮
        			...
        			icons: {
        				...
        				triggerSearch: 'glyphicon-search',
