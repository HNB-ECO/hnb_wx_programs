...
$().ready(function(){
    ...
    window.operateEvents = {
        ...
        'click .DetailOfedit': function (e, value, row, index) {
            ...
            $("#ucoin").val(row.coin)
            $("#uwithdrawCoin").val(row.withdrawCoin)
            $("#uplatformName").val(row.platformName)
            $("#uplatformOwner").val(row.platformOwner)
            $("#update").on('click',function(){
                $.ajax({
                    ...
                    data: {
                        id:row.id,
                        platformName: $("#uplatformName").val(),
                        platformOwner:$("#uplatformOwner").val(),
                        coin:$("#ucoin").val(),
                        withdrawCoin:$("#uwithdrawCoin").val(),
    ...
    $table.bootstrapTable({
        url: "/admin/platform/pageAll",//加载数据的url
        // uniqueId: "goodNameId",
        ...
        columns: [
            ...
            },
            {
                ...
                formatter:  function operateFormatter(value, row, index) {
                    return [
                        '<input type="submit" value="更新信息" class="DetailOfedit btn btn-warning  btn-sm"   data-toggle="modal"  style="display:inline-block;margin-left: 2px" >',
                    ].join('');
                }
            }
...
var saveFormValidate = function() {
    $("#saveModal form").validate({
        submitHandler: function(form) {   //表单提交句柄,为一回调函数，带一个参数：form
            $.ajax({
                url: "/admin/platform/createPlatform",
                type: "post",
                cache: false,
                data: $(form).serialize(),
                //dataType: "json",
